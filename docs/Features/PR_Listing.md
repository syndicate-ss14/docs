...
All features that are not purely enhancements or direct upgrades of features are disabled by default.

## PR 4 - Salvage Ticket System

Tickets are disabled by default. To enable them: 
1. Have mappers replace the existing SalvageVendor with a SalvageTicketVendor. This will be immobile and force salvagers to return to the station to deposit tickets. Recommended to simply make a migration to swap prototypes.
